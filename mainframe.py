...
def check_matching(source_img1,source_img2, model_path, Target_cls_to_ignore=3):
    ...
    results1 = model(img1)
    results2 = model(img2)
    bounding_box_list_of_lists = []
    duplicate_bounding_box_set = set()
    final_list
    for r1,r2 in zip(results1,results2):
        for box1 in r1.boxes:
            ...
            if class_id1 != int(Target_cls_to_ignore):
                ...
                bounding_box_list_of_points = [x11,y11,x21,y21]
                bounding_box_list_of_lists.append(bounding_box_list_of_points)
            for box2 in r2.boxes:
                class_id2 = int(box2.cls.item())
                if class_id2 != int(Target_cls_to_ignore):
                    if (x11,y11,x21,y21) == map(int, box2.xyxy[0]):
                        duplicate_bounding_box = [x11,y11,x21,y21]
                        duplicate_bounding_box_set.add(duplicate_bounding_box)   
                    else:
                        pass
        for items in bounding_box_list_of_lists:
